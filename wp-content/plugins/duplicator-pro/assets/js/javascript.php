...
(function ($) {
    ...
    DupPro.isInViewport = function ( $element ) {
    ...
    DupPro.addAdminMessage = function (message, errLevel, options) {
        ...
        var noticeCLasses = 'dpro-admin-notice notice ' + classErrLevel + ' no-display';
...
jQuery(document).ready(function ($)
{
    ...
    */
    if($('.dup-nav-item:last-child').length > 0) {
        ...
        if (!DupPro.isInViewport($lastTab)) {
            $header.addClass('dup-scrollable-header');
        }

        $header.on('scroll', function() {
            $header.toggleClass('dup-scrollable-header', !DupPro.isInViewport($lastTab));
        });
    }
