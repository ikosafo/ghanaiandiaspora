 ;(function ($) {
    ...
    $(document).ready(function () {
        /***********************************
         charifund Custom widget js
         ************************************/
        $(document).on('widget-updated',function (e,widget) {
            var widget_id = $(widget).attr('id');
            if (widget_id.indexOf('charifund_about_us_widget') != 1){
                prefetch();
            }
        });

        $('body').off('click','.charifund_flogo_uploader');

        $('body').on('click','.charifund_flogo_uploader',function (e) {
            var el = this;
            ...
            file_uploder_frame.on('insert',function () {
                ...
                var img_prev_container = $(el).siblings('.charifund-logo-preview');
                ...
                $(el).siblings().children('.charifund_logo_id').val(selected_ids.join(","));
                $(el).siblings().children('.charifund_logo_id').trigger('change');
            ...
            file_uploder_frame.on('open',function () {
                ...
                console.log($(el).siblings().children('.charifund_logo_id').val());
                var atts = $(el).siblings().children('.charifund_logo_id').val().split(",");
        ...
        function prefetch () {
            $('.charifund_logo_id').each(function () {
                var attid = $(this).val();
                var container = $(this).parent().siblings('.charifund-logo-preview');
                container.html('');

                if ( attid ){
                    $(this).parent().parent().find('.charifund_flogo_uploader').val('Change Image');
