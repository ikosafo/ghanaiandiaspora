...
function generateMembershipCardPDF($member, $memId) {
    ...
    $pdf->SetXY(22, 40);  $pdf->Cell(22, 4, $member['dob'] ?? '12.10.1985', 0, 0);
    ...
    $pdf->SetXY(4, 47.5); $pdf->Cell(40, 4, $member['passport_number'] ?? '---', 0, 1);
    ...
    $pdf->SetXY(4, 43);
    $dob = date('ymd', strtotime($member['dob'] ?? '1985-10-12'));
...
if ($id > 0 && $action === 'approve') {
    ...
    try {
        ...
        $mail->Body = "
            ...
            <p>Your official membership card is attached (valid for 5 years).</p>
            <p>Thank you for being part of the Ghanaian Diaspora Union in Europe!</p>
        ";
